...
<div class="card">
    ...
    <?php if(isset($_SESSION['eo'])){ 
      ...
      $select = mysqli_query($conn, "SELECT * FROM sewa 
      INNER JOIN kios ON kios.id_kios=sewa.id_kios
      INNER JOIN jenis_sewa ON jenis_sewa.id_jenis_sewa=sewa.id_jenis_sewa
      INNER JOIN penyewa ON penyewa.id_penyewa=sewa.id_penyewa
      WHERE id_status='3'");
              ...
              <div class="card-body table-responsive p-0">
                <table class="table table-striped table-valign-middle">
                  ...
                  <tr>
                    ...
                    <td><?= $sel['nama_kios'];?></td>
                    <td><?= $sel['nama_penyewa'];?></td>
                    <td><?= $sel['nama_jenis_sewa'];?></td>
                    <td><?= $sel['tgl_awal_sewa'];?></td>
                    <td><?= $sel['tgl_akhir_sewa'];?></td>
                    <td><?= $status?></td>
                    <td>
                        <a onclick="return confirm('Apakah anda yakin ACC pesanan?')" href="controller/acc_sewa.php?id=<?= $id;?>" class="btn btn-primary"><i class="fa fa-thumbs-up"></i></a>
                    </td>
